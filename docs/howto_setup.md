...
本書ではWindows10からRaspberry PiにSSH接続する手順を記載する。  
Windowsキーを押してPowershellと入力して検索し、Powershell(図X)を起動する。  
![](./img/howto/powershell.PNG)  
Powershellを起動したら、`$ ssh pi@<Raspberry PiのIPアドレス>`を実行してSSH接続する。  
Are you sure you want to continue connectingと聞かれるのでyesでEnterを押す。  
Raspberry Piのpiユーザのパスワードを入力し、ログインする。  
...
6. 使用方法  
led-matrixの使用方法について解説する。  
http://<Raspberry PiのIPアドレス>/へアクセスすると、図Xのようなトップ画面が表示される。  
左上のハンバーガーメニューを展開して、各機能にアクセスする。  
  1. Text
  テキスト表示機能は最も基本的な機能で入力されたテキストをスクロールし続ける機能である。
  http://<Raspberry PiのIPアドレス>/#/textまたは、メニュー中のTextから利用できる。  
  画面構成は図Xのようになっている。  
  上部のテキストボックスに表示するテキストを入力し、その右にある送信ボタンを押下することでLEDパネルにテキストが表示される。文字数が多い場合は表示まで数秒から数十秒間待機する必要がある。
  下部のSpeedのスライダを調整すると、1-100までの100段階でスクロール速度を調整できる。  
  Colorのカラーピッカーを操作すると、表示中のテキストの文字色を変更することができる。  
  画面最上部右のボタンは一時停止ボタンと停止ボタンであり、スクロール中のテキストに適用される。  
  既にテキストを表示中の場合は停止ボタンを押してから新しいテキストを送信しなければいけない。  
  過去に入力したテキストはテキストボックスをクリックすると現れるリストに表示されているので再度表示可能である。  
  2. Phrase
  定型文機能は予め定型文を登録してそれをよびだすことができる機能である。  
  http://<Raspberry PiのIPアドレス>/#/text/phraseまたは、メニュー中のPhraseから利用できる。  
  画面構成は図Xのようになっており、画面最下部の+ボタンを押して定型文を登録する。  
  登録メニューは図Xのようになっており、テキストボックスに登録するテキストを入力してADDボタンを押して登録する。  
  登録した各項目に対して、削除、編集機能を備える。  
  3. RSS
  RSS機能はインターネット上で提供されているRSSを取得してLEDパネルで表示する機能である。  
  http://<Raspberry PiのIPアドレス>/#/text/rssまたは、メニュー中のRSSから利用できる。  
  画面構成は図Xのようになっており、定型文機能同様にRSSを登録する。  
  登録メニューは図Xのようになっており、Nameには任意の識別できる配信元の名前を入力し、URLにはRSSを配信しているURLを入力する。  
